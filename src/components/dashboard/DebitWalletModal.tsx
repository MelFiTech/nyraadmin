...
export const DebitWalletModal = ({ isOpen, onClose, onSuccess }: DebitWalletModalProps) => {
  const { token } = useAuth();
  ...
  const handleTransfer = async () => {
    ...
    try {
      ...
      const response = await axios.post<TransferResponse>(
        `${process.env.NEXT_PUBLIC_API_URL}/admin/wallet/user/${selectedUser.user_id}/debit`, 
        {
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose}>
      <div className="space-y-4">
        <h2 className="text-xl font-bold text-dark">Debit a Wallet</h2>
        ...
        <div className="flex justify-end space-x-3 mt-6 w-full">
          ...
          >
            {isLoading ? 'Processing...' : 'Debit Wallet'}
          </button>
        </div>
      </div>
    </Modal>
